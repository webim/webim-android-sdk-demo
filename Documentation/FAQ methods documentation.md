...
`rootId` – **ID** вершины, которая будет корнем дерева (структуры) блока часто задаваемых вопросов. Тип – **int**.
...
`id` – **ID** категории блока часто задаваемых вопросов. Тип – **int**.
...
## `void getCategoriesForApplication(String application, String language, String, departmentKey, GetCallback<List<Integer>> callback)`

Метод, который используется для получения массива идентификаторов категорий, подходящих для заданных параметров, через **GetCallback\<List\<Integer\>\>**. Если запрос завершился с ошибкой, то полученный массив будет пустой.

**Важно:** для правильной работы метода требуется вызов метод **resume()**.

### Параметры

`application` – название приложения. Тип – **String**.

`language` – язык. Тип – **String**.

`departmentKey` – ключ отдела. Тип – **String**.

`callback` – объект, реализующий интерфейс **GetCallback\<List\<Integer\>\>**.
...
## `void search(String query, int id, int limit, GetCallback<FAQSearchItem> callback)`
...
`id` – **id** категории, в которой осуществляется поиск. Тип – **int**.

`limit` – число, не больше которого нужно вернуть статей.
